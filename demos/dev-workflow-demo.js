...
const plan = await agent(`你是一位 Tech Lead。需求如下：
...
// 第一轮：并行实现
implementations = await parallel(plan.tasks.map((task) => () =>
  agent(`你是 ${task.name} 开发者。

## 你的任务
实现文件 \`${task.file}\`。

## 需求
${task.functions && task.functions.length > 0
    ? task.functions.map((f) => `- ${f}`).join("\n")
    : task.description}
...
for (let round = 0; round < MAX_RETRIES; round++) {
  ...
  reviewResult = await agent(`你是一位 Code Reviewer。请审查以下实现：
...
// 收集最终代码
const allCode = implementations
  .filter(Boolean)
  .map((m) => ({ file: m.file, content: m.code }));
...
const summary = await agent(`你是 Tech Lead。开发工作流已完成，请生成最终总结。
